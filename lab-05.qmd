...
camels_feat <- camels %>% 
  select(all_of(soil_vars), all_of(geol_vars), all_of(veg_vars), all_of(hydro_vars), all_of(climate_vars), all_of(vars_loc), gauge_lat, gauge_lon, q_mean) %>% 
...
No features are normal, all require a transformation
...
camels_recipe <- recipe(log_qmean ~ ., data = camels_train) %>% 
  step_rm(gauge_lon, gauge_lat) %>% #remove lat and lon from formula
