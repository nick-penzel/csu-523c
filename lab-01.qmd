...
max_q_table <- daily_q %>% filter(Date == my.date) %>% 
  ...
  set_caption(caption = paste("Top 5 sites with highest flow,", my.date)) %>% 
  autofit()
...
max_qdelta_table <- daily_q %>% filter(Date == my.date) %>% 
  ...
  set_caption(caption = paste("Top 5 sites with highest daily flow increase,", my.date)) %>% 
  autofit()
...
df %>% 
  filter(Date == my.date,
         site_no %in% c("09380000", "09163500", "09085000", 
                        "09306500", "09379500", "09402500")) %>% 
...
#   need to come back and asnwere part c 
```
...
thresh_comp <-  df %>% 
  ...
  ungroup() %>% 
  group_by(period) %>%
  summarise(avg_days_below = mean(days_below))
...
year_df <- df %>% 
  ...
  summarise(
    annual_q = sum(Flow),
    peak_q = max(Flow),
    days_below_threshhold = sum(below_threshold)
  ) %>% 
  ungroup()
```
...
year_df %>% 
  filter(site_no == "09380000",
           water_year >= 1980 & water_year <= 2023) %>% 
  ggplot(aes(x = water_year, y = annual_q, fill = annual_q >= lees_median))+
    geom_col()+
